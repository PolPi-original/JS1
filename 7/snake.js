...
let snake = {
    ...
    makeStep(settings) {
        ...
        this.body.unshift(this.getNextStepHeadPoint(settings));
        this.body.pop();
    },

    getNextStepHeadPoint(settings) {
        ...
        switch (this.direction) {
            case 'up':
                ...
                if (nextPoint.y < 0) {
                    nextPoint.y = settings.rowsCount - 1;
                }
                break;
            case 'down':
                nextPoint = {x: firstPoint.x, y: firstPoint.y + 1};
                if (nextPoint.y >= settings.rowsCount) {
                    nextPoint.y = 0;
                }
                break;
...
let renderer = {
    ...
    renderScope(scope) {
        let scopeGame = document.getElementById('scope');
        scopeGame.innerText = 'Съедено еды: ' + scope;
    },
...
let game = {
    ...
    tickHandler() {
        ...
        if(this.food.isFoodPoint(this.snake.getNextStepHeadPoint(this.settings))) {
        ...
        this.snake.makeStep(this.settings);
        this.renderer.render(this.snake.body, this.food.getFoodCoordinates());
        this.renderer.renderScope(this.scope);
    },
    ...
    canSnakeMakeStep() {
        let nextHeadPoint = this.snake.getNextStepHeadPoint(this.settings);
...
window.onload = function () {
    game.init({speed: 3, winLength: 20, rowsCount: 10, colsCount: 10});
};
